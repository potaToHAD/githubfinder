    ...
    .then((data) => {
      if (data.message) {
        ...
        document.getElementById(
          "profile"
        ).innerHTML = `<img   style="width: 80px;height:90px;padding-top:10px; border-radius: 40%;border-radius: 50%;" src="${data.avatar_url}">`;
        ...
        document.getElementById(
          "repo"
        ).innerHTML = `<p>Repos - ${data.public_repos} </p>`;
        document.getElementById(
          "follower"
        ).innerHTML = `<p>Follower - ${data.followers} </p>`;
        document.getElementById(
          "following"
        ).innerHTML = `<p>Follower - ${data.following} </p>`;
        document.getElementById(
          "gitid"
        ).innerHTML = `<a>  ${data.location} </a>`;
        document.getElementById(
          "twit"
        ).innerHTML = `<a>  ${data.twitter_username} </a>`;
      }
    })
    .catch((e) => {
      console.log(e);
